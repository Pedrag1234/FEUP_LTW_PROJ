...
function refresh() {
  ...
  request.open('get', '../actions/sendmessage.php?' + encodeForAjax({'last_id': last_id}), true);
...
function sendMessage(event) {
  ...
  // Send message
  let request = new XMLHttpRequest();
  request.open('get', '../actions/sendmessage.php?' + encodeForAjax({'last_id': last_id, 'Username': username, 'text': message}), true);
  request.addEventListener('load', messagesReceived);
  request.send();
